...
export function CareerHub() {
  ...
  const renderCard = (opp: any) => (
    <Card key={opp.id} className="flex flex-col h-full bg-slate-900/50 border-slate-800 hover:border-indigo-500/50 hover:shadow-[0_0_20px_rgba(99,102,241,0.1)] transition-all group overflow-hidden relative">
      ...
      <CardContent className="p-6 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-4">
          <div className="flex flex-wrap gap-2 mb-2">
            ...
            <span className="px-2.5 py-1 rounded-md text-xs font-semibold uppercase tracking-wider bg-slate-800 text-slate-300 border border-slate-700">
              {opp.category}
            </span>
          </div>
        ...
        <h3 className="text-xl font-bold text-white mb-2 line-clamp-2 group-hover:text-indigo-300 transition-colors">{opp.title}</h3>
        
        <div className="flex items-center gap-3 mb-4 text-sm font-medium">
          <span className="text-indigo-400 bg-indigo-500/10 px-2 py-1 rounded">{opp.platform}</span>
          <span className="text-slate-500">•</span>
          <span className="text-slate-400">{opp.duration || 'Flexible'}</span>
        </div>
        
        <p className="text-sm text-slate-400 mb-6 line-clamp-3 flex-1 leading-relaxed">{opp.description}</p>
        ...
        <button 
          onClick={() => window.open(opp.externalLink, '_blank')}
          className="w-full mt-auto flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white px-4 py-2.5 rounded-lg text-sm font-semibold shadow-lg shadow-indigo-500/20 transition-all"
        >
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} className="w-full bg-[#0b1220] border border-slate-700 rounded-md px-3 py-2 text-white focus:outline-none focus:border-indigo-500" placeholder="e.g. 3 Months, Self-paced" />
                </div>

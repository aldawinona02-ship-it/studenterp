...
import { X, Edit2, Trash2, User, AlertTriangle } from 'lucide-react';
...
export function Students() {
    ...
    fetch('/api/students')
      ...
      .then(data => {
        const formattedData = data.map((s: any) => ({
          ...s,
          studentId: s.id.toString().padStart(6, '0'),
  ...
  const checkAndSendAlert = async (studentName: string, studentId: string, department: string, ssisScore: number, riskLevel: string) => {
    if (riskLevel === 'High Risk') {
      try {
        const res = await fetch('/api/admin/teachers');
        const teachers = await res.json();
        const teacher = teachers.find((t: any) => t.department === department);
        ...
        if (teacher && teacher.email) {
          ...
          setNotificationMessage('Email alert sent to teacher for high-risk student.');
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    await checkAndSendAlert(newStudent.name, newStudent.studentId, newStudent.department, newStudent.ssis_score, riskLevel);
  };
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    await checkAndSendAlert(formData.name, formData.id, formData.department, ssis, riskLevel);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {notificationMessage && (
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 flex items-center gap-3 animate-in slide-in-from-top-4 duration-500">
          <div className="w-8 h-8 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <p className="text-emerald-400 font-medium">{notificationMessage}</p>
        </div>
      )}
                    ...
                    <td className="px-4 py-3 font-medium text-slate-200">
                      <div className="flex items-center gap-2">
                        ...
                        {student.risk_level === 'High Risk' && (
                          <AlertTriangle className="w-4 h-4 text-red-500 ml-1" title="High Risk Student" />
                        )}
                      </div>
                    </td>
